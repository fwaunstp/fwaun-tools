//! Qwen3-VL ONNX captioner (default: `onnx-community/Qwen3-4B-VL-ONNX`,
//! 4B vision-fp32 / text-int4 variant).
//!
//! Three sessions:
//! ```text
...
pub enum CaptionerError {
    ...
    Shape(String),
    #[error(
        "tokenized chat template did not contain exactly one <|image_pad|> (id {IMAGE_PAD_TOKEN_ID}); got {0}"
    )]
    ImagePadCount(usize),
}
